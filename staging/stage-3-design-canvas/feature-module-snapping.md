...
- **Seam classification** (`computeSeams`) — this is the join-type detection Stage 4's auto-BOM depends on:
  ...
  - A seam where either side is a `gangway`-category item is flagged `isGangwayAttachment: true` and skips `CONNECTOR_RULES` entirely — gangway attachment uses dedicated hinge hardware, not the standard module connectors. See `feature-gangways.md`.
- **Selection**: after a real drag (movement occurred), the moved item stays selected — Rotate/Delete work immediately, no extra click needed. Fixed a real bug where a plain click (select without dragging) would immediately deselect again: the click event that fires after pointerdown+pointerup bubbles to the canvas's background-click-to-deselect handler, since `stopPropagation()` on the pointerdown event doesn't stop the later, separate `click` event. Fixed by only deselecting when the click's `event.target` is the canvas background itself, not a bubbled child.
